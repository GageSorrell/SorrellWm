...
inline void horizontal_blur_extend(const T * in, T * out, const int w, const int h, const int r)
{
    ...
    // std::cout << "Kernel is " << kernel << " of kernel small (" << kSmall << "), mid (" << kMid << "), and large (" << kLarge << ")." << std::endl;
    #pragma omp parallel for
    for(int i=0; i<h; i++)
    {
        // std::cout << "Getting close A" << std::endl;
        const int begin = i*w;
        const int end = begin+w;
        calc_type fv[C], lv[C], acc[C];                 // first value, last value, sliding accumulator
        // std::cout << "Getting close B, begin " << begin << ", end " << end << std::endl;

        // init fv, lv, acc by extending outside the image buffer
        for(int ch=0; ch<C; ++ch)
        {
            // std::cout << "Getting close C, ch " << ch << std::endl;
            fv[ch] =  in[begin*C+ch];
            // std::cout << "Getting close C 01, end-1*C+ch " << end-1*C+ch << " BUT " << Width * Height * 3 << std::endl;
            lv[ch] =  in[(end-1)*C+ch];
            // std::cout << "Getting close C 02, ch " << ch << std::endl;
            acc[ch] = (r+1)*fv[ch];
            // std::cout << "Getting close C 03, ch " << ch << std::endl;
        }
        ...
        else if constexpr(kernel == kSmall)
        {
            ...
            // std::cout << "ti " << ti << ", li " << li << ", ri " << ri << std::endl;

            // initial accumulation
            for(int j=ti; j<ri; j++)
            for(int ch=0; ch<C; ++ch)
            {
                // std::cout << "j " << j << ", ch " << ch << std::endl;
                acc[ch] += in[j*C+ch];
            }
            ...
            for(int ch=0; ch<C; ++ch)
            {
                // std::cout << "li, ri, ti: " << li << ", " << ti << ", " << li << std::endl;
                // std::cout << "in[ri*C+ch] " << in[ri*C+ch] << ", fv[ch] " << fv[ch] << std::endl;
                acc[ch] += in[ri*C+ch] - fv[ch];
                // assert(acc[ch] >= 0);
                // std::cout << "acc[ch] " << acc[ch] << ", acc[ch]*iarr " << acc[ch]*iarr << std::endl;
                out[ti*C+ch] = acc[ch]*iarr + round_v<T>();
            }

            // std::cout << "Completed Step 1!" << std::endl;
            ...
            // std::cout << "Completed Step 2!" << std::endl;
            ...
            // std::cout << "Completed Step 3 of element " << i << " of " << h << " total elements" << std::endl;
        }
    }
    // std::cout << "Completed the for loop!" << std::endl;
}
...
//!
//! \brief This function performs a 2D transposition of an image.
//!
...
inline void fast_gaussian_blur(T *& in, T *& out, const int w, const int h, const int c, const float sigma)
{
    ...
    for(int i = 0; i < N; ++i)
    {
        // std::cout << in << " " << out << " " << w << " " << h << " " << c << " " << boxes[i] << std::endl;
