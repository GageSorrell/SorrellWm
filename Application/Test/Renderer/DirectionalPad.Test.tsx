...
describe("DirectionalPad.GetTintedFill", () =>
{
    it.each([ ColorScheme.Dark, ColorScheme.Light ])(
        ...
        (Scheme: ColorScheme) =>
        {
            const Background = Unwrap(RgbColor.From.Hex(AcrylicBaseColorHex[ Scheme ]));
            const HighContrastTint = Scheme === ColorScheme.Dark
                ? "rgb(255, 255, 255)"
                : "rgb(0, 0, 0)";

            const Fill = GetTintedFill(HighContrastTint, 0, Scheme);
            const FillColor = Unwrap(RgbColor.From.Hex(Fill));
    ...
    it.each([ ColorScheme.Dark, ColorScheme.Light ])(
        ...
        {
            const Background = Unwrap(RgbColor.From.Hex(AcrylicBaseColorHex[ Scheme ]));
            const Inactive = Unwrap(RgbColor.From.Hex(InactiveForegroundColorHex[ Scheme ]));
            ...
            const RawFillColor = Unwrap(RgbColor.From.Hex(LowContrastTint));
