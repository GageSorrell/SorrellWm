 ...
 *
 * @module @sorrell/wm/Main/TilingManager
 *
 * @file      TilingManager.ts
 * @author    Gage Sorrell <gage@sorrell.sh>
 * @copyright (c) 2026 Gage Sorrell
 * @license   MIT
 */

import * as TilingTree from "./TilingTree.js";
...
const TypeId = "~sorrell/wm/Main/TilingManager" as const;
...
export interface Dependencies
{
    ...
    readonly Enumerate: () => Result.Result<ReadonlyArray<Handle.HWND>, NativeFailure>;
    ...
    readonly SetWindowRect: (
        ...
        Bounds: Box.Box
    ) => Result.Result<void, NativeFailure>;
}
    ...
    Effect.gen(function*()
    {
        ...
        const Apply = (
            ...
            (Placement: TilingTree.Placement) => Effect.sync(() =>
                ...
                Effect.mapError((Failure: NativeFailure) => new WindowLayoutError({
        ...
        const Discover = Effect.sync(DependenciesValue.Enumerate).pipe(
            Effect.flatMap(Effect.fromResult),
            Effect.mapError((Failure: NativeFailure) => new WindowEnumerationError({
                Message: Failure.Message
            })),
        ...
        const Manage: TilingManagerImpl["Manage"] = (
            WindowValue: Handle.HWND,
            Target?: Handle.HWND,
            Orientation?: TilingTree.Orientation
        ) =>
        ...
        const Unmanage: TilingManagerImpl["Unmanage"] = (
            WindowValue: Handle.HWND,
        ...
        return {
            ...
            Manage,
            Move,
            Reconcile: SubscriptionRef.get(StateRef).pipe(Effect.flatMap(Apply)),
            Refresh,
            SetPanelOrientation,
            SetPanelRatio,
