...
const ValidatorDescriptions = {
    "config-not-missing-providers": "Determine whether the consumer's config has an entry for every installed provider.",
    ...
    "no-collisions": "Determine whether any augmented interface would have more than one " +
        "property of the same key, if the `generate` command were run.",
    "no-ignored-providers": "Determine whether there is an installed provider that is not currently used " +
...
export const ValidateConfig = MakeConfig({
    Fix: pipe(Cli.Options.Choice.Choice("fix", FixCategories, {
        Base: "@TODO",
        Choices: {
            all: "@TODO",
            error: "@TODO",
    ...
    }), Options.withDefault(["all"]), Options.optional),
    Validators: pipe(Cli.Args.Choice.Choice(Validate.Choices, {
        Description: {
            Base: "The validators to run when calling this command.  " +
