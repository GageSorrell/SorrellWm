 ...
 * @license   MIT
 */
import { Config as EffectConfig, pipe } from "effect";
 ...
 */
export function IsValid(CodeAugerProperty) {
    return (typeof CodeAugerProperty === "object" &&
...
export const ExportedType = EffectConfig.all({
    Name: pipe(EffectConfig.nonEmptyString("name"), EffectConfig.withDescription(`The ${Code("export")}ed name of the ${Code("type")}/${Code("interface")} ` +
        "described by this.")),
    Path: pipe(EffectConfig.nonEmptyString("path"), EffectConfig.withDescription(""
...
export const Config = EffectConfig.all({
    GenericProperty: pipe(ExportedType, EffectConfig.nested("genericProperty"), EffectConfig.withDescription(""
    ...
    Interface: pipe(ExportedType, EffectConfig.nested("interface"), EffectConfig.withDescription(`The exported ${Code("interface")} that is augmented with properties ` +
        "whose types are derived from the (generic) property type given by the other " +
        "property in this object.")),
    "~Schema": pipe(EffectConfig.nonEmptyString("$schema"), EffectConfig.withDefault("code-auger/provider/schema"), EffectConfig.withDescription(`The ${Code("JSON")} schema for this config file.`))
});
