...
export /**
    ...
    Name: EffectSchema.NonEmptyString.annotateKey({
        description: (`The ${Code("export")}ed name of the ${Code("type")}/${Code("interface")} ` +
            "described by this."),
        title: "name"
    }),
    Path: EffectSchema.NonEmptyString.annotateKey({
        description: ("The path specified in the provider's `package.json` \"exports\" " +
            "property, *i.e.*, the relevant property key from which the interface " +
            "to augment is exported."),
        title: "path"
    }).annotate({
        description: ("A basic description of a given type that `code-auger` uses to augment a provider's " +
            "\"type registry\" with consumer-defined types."),
...
export /**
    ...
    GenericProperty: ExportedType
        .annotateKey({
        description: ("The name of the generic type that the provider exports, such that *concrete* " +
    ...
    Interface: ExportedType
        .annotateKey({
        description: ("The exported interface that is augmented with properties whose types are derived from the " +
            "(generic) property type given by the other property in this object."),
        title: "interface"
    })
}).annotate({
    description: ("The configuration that informs `code-auger` how it should generate the module augmentation for " +
