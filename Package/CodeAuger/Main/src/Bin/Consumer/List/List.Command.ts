...
import { Console, Effect, type FileSystem, type Path as PathService, pipe } from "@sorrell/effect";
...
const InstalledProviders: Effect.Effect<
...
> = Effect.gen(function* ()
{
    ...
    >
    {
        return pipe(
            ...
            Effect.catch((_In: Effect.Effect<boolean, any, PathService.Path | FileSystem.FileSystem>) =>
            {
...
function PrintList(Options: Handler.Argument<typeof ListConfig>)
{
    return Effect.gen(function* ()
    {
        if (Options.Rich)
        {
            const Term: Terminal = yield* Terminal;
            yield* pipe(
                InstalledProviders,
                Effect.flatMap(Effect.forEach(Term.display))
            );
            ...
            yield* pipe(
                InstalledProviders,
                Effect.flatMap(Effect.forEach(Console.log))
            );

            yield* Term.display("Foo\n\n");
            yield* Console.log("Foo\n\n");

            yield* InstalledProviders.pipe(
                Effect.flatMap(
                    Effect.forEach((Element) =>
                        Term.display(Element)
                    )
                )
            );
...
export const ListCommand: Subcommand<"ls", typeof ListConfig> =
    Command.make("ls", ListConfig, HandleList);
