...
Schema-validated, JSON-file-backed application settings for [`effect`](https://npmjs.com/package/effect).
...
The layer requires Effect's `FileSystem` and `Path` services.  This package was piloted and tested with `@effect/platform-node` in [SorrellWm](https://github.com/GageSorrell/SorrellWm), but it is expected that other platforms may be used without issue.
...
| Linux    | `$XDG_CONFIG_HOME/SorrellWm/settings.json` or<br /> `~/.config/SorrellWm/settings.json` |
...
- A failed write returns `AppSettings.WriteError` and leaves the last committed in-memory value unchanged
