...
/** Replace with latest once it is released. */
const tag = "latest";
...
function createApiPage(pluginId, readme, pkgJson)
{
    ...
    const editApiUrl =
        `https://github.com/ionic-team/capacitor-plugins/blob/main/${ pluginId }` +
        "/src/definitions.ts";
     ...
     * - Escape `{` characters inside <code> blocks because MDX treats `{}` as JavaScript expressions.
     *   Unescaped `{` inside code blocks can cause parsing errors.
     *
     * - Convert JSDoc-style {@link URL|Text} and {@link URL} to proper Markdown links:
     *   - {@link URL|Text} → [Text](URL)
     *   - {@link URL} → [URL](URL)
     *   This is necessary because MDX does not understand the JSDoc `@link` syntax,
     *   and leaving it unconverted will cause parsing errors.
     */
    readme = readme
