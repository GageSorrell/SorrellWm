 ...
 * @category tags
 */
export const Socket = Context.GenericTag("@effect/platform/Socket");
/**
 * @since 1.0.0
 * @category type ids
 */
 ...
 * @category combinators
 */
export const toChannelString = dual((args) => isSocket(args[0]), (self, encoding) => {
 ...
 * @category tags
 */
export const WebSocket = Context.GenericTag("@effect/platform/Socket/WebSocket");
/**
 * @since 1.0.0
 * @category tags
 */
export const WebSocketConstructor = Context
    .GenericTag("@effect/platform/Socket/WebSocketConstructor");
/**
 * @since 1.0.0
 * @category layers
 */
export const layerWebSocketConstructorGlobal = Layer.succeed(WebSocketConstructor, (url, protocols) => new globalThis.WebSocket(url, protocols));
/**
 * @since 1.0.0
 * @category constructors
 */
...
export const fromWebSocket = (acquire, options) => Effect.withFiberRuntime((fiber) => {
    ...
    const runRaw = (handler, opts) => Effect.scopedWith(Effect.fnUntraced(function* (scope) {
        ...
        function onError(cause) {
            ...
            Deferred.unsafeDone(fiberSet.deferred, Effect.fail(new SocketGenericError({ reason: open ? "Read" : "Open", cause })));
        }
 ...
 * @category fiber refs
 */
export const currentSendQueueCapacity = globalValue("@effect/platform/Socket/currentSendQueueCapacity", () => FiberRef.unsafeMake(16));
/**
 * @since 1.0.0
 * @category constructors
 */
export const fromTransformStream = (acquire, options) => Effect.withFiberRuntime((fiber) => {
    ...
    const write = (chunk) => latch.whenOpen(Effect.suspend(() => {
        ...
        if (isCloseEvent(chunk)) {
            return Deferred.fail(fiberSet.deferred, new SocketCloseError({ reason: "Close", code: chunk.code, closeReason: chunk.reason }));
        }
