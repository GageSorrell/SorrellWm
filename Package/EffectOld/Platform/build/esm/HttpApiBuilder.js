 ...
 * @category router
 */
export class Router extends HttpRouter.Tag("@effect/platform/HttpApiBuilder/Router")() {
}
/**
 ...
 */
export const api = (api) => Layer.effect(HttpApi.Api, Effect.map(Effect.context(), (context) => ({ api: api, context })));
/**
 ...
 * @category constructors
 */
export const buildMiddleware = Effect.fnUntraced(function* (api) {
 ...
 * @category handlers
 */
export const HandlersTypeId = Symbol.for("@effect/platform/HttpApiBuilder/Handlers");
const HandlersProto = {
    [HandlersTypeId]: {
        _Endpoints: identity
    },
...
const handlerToRoute = (endpoint_, middleware, handler, isFullRequest, uninterruptible) => {
    ...
    const isMultipartStream = endpoint.payloadSchema.pipe(Option.map(({ ast }) => HttpApiSchema.getMultipartStream(ast) !== undefined), Option.getOrElse(constFalse));
    const multipartLimits = endpoint.payloadSchema.pipe(Option.flatMapNullable(({ ast }) => HttpApiSchema.getMultipart(ast) || HttpApiSchema.getMultipartStream(ast)));
    ...
    }).pipe(Effect.catchIf(ParseResult.isParseError, HttpApiDecodeError.refailParseError))), { uninterruptible });
};
        ...
        if (entry.tag.optional) {
            ...
            handler = Effect.matchEffect(effect, {
                ...
                onSuccess: entry.tag.provides !== undefined
                    ? (value) => Effect.provideService(previous, entry.tag.provides, value)
                    : (_) => previous
            });
        }
        else {
            handler = entry.tag.provides !== undefined
                ? Effect.provideServiceEffect(handler, entry.tag.provides, effect)
                : Effect.zipRight(effect, handler);
        }
    }
    return handler;
};
const securityMiddlewareCache = globalValue("securityMiddlewareCache", () => new WeakMap());
...
export class Middleware extends Context.Tag("@effect/platform/HttpApiBuilder/Middleware")() {
     ...
     * @since 1.0.0
     */
    static layer = Layer.sync(Middleware, () => {
...
export const middleware = (...args) => {
    ...
    return (Effect.isEffect(middleware)
        ? Layer.scopedDiscard(Effect.flatMap(middleware, add))
        : Layer.scopedDiscard(add(middleware))).pipe(Layer.provide(Middleware.layer));
};
/**
...
export const securityDecode = (self) => {
    switch (self._tag) {
        ...
        case "ApiKey": {
            ...
            const decode = unify(self.in === "query"
