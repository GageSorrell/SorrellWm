...
/** @internal */
export const TypeId = Symbol.for("@effect/platform/HttpRouter");
/** @internal */
export const RouteTypeId = Symbol.for("@effect/platform/HttpRouter/Route");
/** @internal */
export const RouteContextTypeId = Symbol.for("@effect/platform/HttpRouter/RouteContext");
/** @internal */
export const RouteContext = Context.GenericTag("@effect/platform/HttpRouter/RouteContext");
...
/** @internal */
export const currentRouterConfig = globalValue("@effect/platform/HttpRouter/currentRouterConfig", () => FiberRef.unsafeMake({}));
/** @internal */
export const withRouterConfig = dual(2, (effect, config) => Effect.locally(effect, currentRouterConfig, config));
/** @internal */
...
export const toHttpApp = (self) => Effect.map(FiberRef.get(currentRouterConfig), (config) => {
    ...
    const mounts = Chunk.toReadonlyArray(self.mounts).map(([path, app, options]) => [
        path,
        new RouteContextImpl(new RouteImpl("*", options?.includePrefix ? `${path}/*` : "/*", app, options?.includePrefix ? Option.none() : Option.some(path), false), {}),
        options
    ]);
    ...
    return Effect.withFiberRuntime((fiber) => {
        ...
        return Effect.locally((route.uninterruptible ?
            handlerResponse :
            Effect.interruptible(handlerResponse)), FiberRef.currentContext, context);
    });
});
...
/** @internal */
export const empty = new RouterImpl(Chunk.empty(), Chunk.empty());
/** @internal */
...
/** @internal */
export const append = dual(2, (self, route) => new RouterImpl(Chunk.append(self.routes, route), self.mounts));
/** @internal */
export const concat = dual(2, (self, that) => concatAll(self, that));
/** @internal */
...
/** @internal */
export const prefixPath = dual(2, (self, prefix) => {
    prefix = removeTrailingSlash(prefix);
    return self === "/" ? prefix : prefix + self;
});
/** @internal */
export const prefixAll = dual(2, (self, prefix) => {
    prefix = removeTrailingSlash(prefix);
...
/** @internal */
export const mount = dual(3, (self, path, that) => concat(self, prefixAll(that, path)));
/** @internal */
export const mountApp = dual((args) => Predicate.hasProperty(args[0], TypeId), (self, path, that, options) => new RouterImpl(self.routes, Chunk.append(self.mounts, [removeTrailingSlash(path), that, options])));
/** @internal */
...
/** @internal */
export const use = dual(2, (self, f) => new RouterImpl(Chunk.map(self.routes, (route) => new RouteImpl(route.method, route.path, f(Effect.flatMap(route.handler, Respondable.toResponse)), route.prefix, route.uninterruptible)), Chunk.map(self.mounts, ([path, app]) => [path, f(app)])));
/** @internal */
export const transform = dual(2, (self, f) => new RouterImpl(Chunk.map(self.routes, (route) => new RouteImpl(route.method, route.path, f(route.handler), route.prefix, route.uninterruptible)), Chunk.map(self.mounts, ([path, app]) => [path, Effect.flatMap(f(app), Respondable.toResponse)])));
/** @internal */
export const catchAll = dual(2, (self, f) => transform(self, Effect.catchAll(f)));
/** @internal */
export const catchAllCause = dual(2, (self, f) => transform(self, Effect.catchAllCause(f)));
/** @internal */
export const catchTag = dual(3, (self, k, f) => transform(self, Effect.catchTag(k, f)));
/** @internal */
export const catchTags = dual(2, (self, cases) => use(self, Effect.catchTags(cases)));
export const provideService = dual(3, (self, tag, service) => use(self, Effect.provideService(tag, service)));
/* @internal */
export const provideServiceEffect = dual(3, (self, tag, effect) => use(self, Effect.provideServiceEffect(tag, effect)));
const makeService = () => {
    let router = empty;
    return {
        addRoute(route) {
