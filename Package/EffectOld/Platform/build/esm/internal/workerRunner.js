...
/** @internal */
export const CloseLatch = Context.Reference()("@effect/platform/WorkerRunner/CloseLatch", {
    defaultValue: () => Deferred.unsafeMake(FiberId.none)
});
/** @internal */
export const layerCloseLatch = Layer.effect(CloseLatch, Deferred.make());
/** @internal */
export const make = Effect.fnUntraced(function* (process, options) {
    ...
    yield* backing.run((portId, [id, kind, data, span]) => {
        ...
        return Effect.withFiberRuntime((fiber) => {
            ...
            let effect = Effect.isEffect(stream) ?
                Effect.flatMap(stream, (out) => pipe(options?.encodeOutput
                    ? Effect.provideService(options.encodeOutput(input, out), Transferable.Collector, collector)
                    : Effect.succeed(out), Effect.flatMap((payload) => backing.send(portId, [id, 0, [payload]], collector.unsafeRead())))) :
                pipe(stream, Stream.runForEachChunk((chunk) => {
            ...
            return Effect.uninterruptibleMask((restore) => restore(effect).pipe(Effect.catchIf(isWorkerError, (error) => backing.send(portId, [id, 3, WorkerError.encodeCause(Cause.fail(error))])), Effect.catchAllCause((cause) => Either.match(Cause.failureOrCause(cause), {
                onLeft: (error) => {
                    ...
                    return pipe(options?.encodeError
                        ? Effect.provideService(options.encodeError(input, error), Transferable.Collector, collector)
                        : Effect.succeed(error), Effect.flatMap((payload) => backing.send(portId, [id, 2, payload], collector.unsafeRead())), Effect.catchAllCause((cause) => backing.send(portId, [id, 3, WorkerError.encodeCause(cause)])));
                },
...
export const makeSerialized = (schema, handlers) => Effect.gen(function* () {
    ...
    return yield* make((request) => {
        ...
        decode(message) {
            return Effect.mapError(parseRequest(message), (cause) => new WorkerError({ reason: "decode", cause }));
        },
        encodeError(request, message) {
            return Effect.mapError(Schema.serializeFailure(request, message), (cause) => new WorkerError({ reason: "encode", cause }));
        },
        encodeOutput(request, message) {
            return Effect.catchAllCause(Schema.serializeSuccess(request, message), (cause) => new WorkerError({ reason: "encode", cause }));
        }
    });
});
/** @internal */
