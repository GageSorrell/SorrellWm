...
/** @internal */
export const TypeId = Symbol.for("@effect/platform/HttpPlatform");
/** @internal */
export const tag = Context.GenericTag("@effect/platform/HttpPlatform");
/** @internal */
export const make = (impl) => Effect.gen(function* () {
    ...
    return tag.of({
        ...
        fileResponse(path, options) {
            return pipe(Effect.bindTo(fs.stat(path), "info"), Effect.bind("etag", ({ info }) => etagGen.fromFileInfo(info)), Effect.map(({ etag, info }) => {
...
/** @internal */
export const layer = Layer.effect(tag, Effect.flatMap(FileSystem.FileSystem, (fs) => make({
    ...
    fileWebResponse(file, status, statusText, headers, _options) {
        return serverResponse.stream(Stream.fromReadableStream(() => file.stream(), identity), { headers, status, statusText });
    }
