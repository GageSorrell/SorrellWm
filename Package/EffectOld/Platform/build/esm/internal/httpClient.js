...
/** @internal */
export const TypeId = Symbol.for("@effect/platform/HttpClient");
/** @internal */
export const tag = Context.GenericTag("@effect/platform/HttpClient");
/** @internal */
export const currentTracerDisabledWhen = globalValue(Symbol.for("@effect/platform/HttpClient/tracerDisabledWhen"), () => FiberRef.unsafeMake(constFalse));
/** @internal */
export const withTracerDisabledWhen = dual(2, (self, pred) => transformResponse(self, Effect.locally(currentTracerDisabledWhen, pred)));
/** @internal */
export const currentTracerPropagation = globalValue(Symbol.for("@effect/platform/HttpClient/currentTracerPropagation"), () => FiberRef.unsafeMake(true));
/** @internal */
export const withTracerPropagation = dual(2, (self, enabled) => transformResponse(self, Effect.locally(currentTracerPropagation, enabled)));
/** @internal */
export const SpanNameGenerator = Context.Reference()("@effect/platform/HttpClient/SpanNameGenerator", {
    defaultValue: () => (request) => `http.client ${request.method}`
});
/** @internal */
export const withSpanNameGenerator = dual(2, (self, f) => transformResponse(self, Effect.provideService(SpanNameGenerator, f)));
const ClientProto = {
    [TypeId]: TypeId,
    pipe() {
...
const responseRegistry = globalValue("@effect/platform/HttpClient/responseRegistry", () => {
...
const scopedRequests = globalValue("@effect/platform/HttpClient/scopedRequests", () => new WeakMap());
/** @internal */
export const make = (f) => makeWith((effect) => Effect.flatMap(effect, (request) => Effect.withFiberRuntime((fiber) => {
    ...
    return Effect.useSpan(nameGenerator(request), { kind: "client", captureStackTrace: false }, (span) => {
...
post, 
/** @internal */
put } = Effect.serviceFunctions(tag);
/** @internal */
export const transform = dual(2, (self, f) => {
    const client = self;
    return makeWith(Effect.flatMap((request) => f(client.postprocess(Effect.succeed(request)), request)), client.preprocess);
});
/** @internal */
export const filterStatus = dual(2, (self, f) => transformResponse(self, Effect.flatMap(internalResponse.filterStatus(f))));
/** @internal */
export const filterStatusOk = (self) => transformResponse(self, Effect.flatMap(internalResponse.filterStatusOk));
/** @internal */
export const transformResponse = dual(2, (self, f) => {
    const client = self;
    return makeWith((request) => f(client.postprocess(request)), client.preprocess);
});
/** @internal */
export const catchTag = dual(3, (self, tag, f) => transformResponse(self, Effect.catchTag(tag, f)));
/** @internal */
export const catchTags = dual(2, (self, cases) => transformResponse(self, Effect.catchTags(cases)));
/** @internal */
export const catchAll = dual(2, (self, f) => transformResponse(self, Effect.catchAll(f)));
/** @internal */
export const filterOrElse = dual(3, (self, f, orElse) => transformResponse(self, Effect.filterOrElse(f, orElse)));
/** @internal */
export const filterOrFail = dual(3, (self, f, orFailWith) => transformResponse(self, Effect.filterOrFail(f, orFailWith)));
/** @internal */
export const mapRequest = dual(2, (self, f) => {
    const client = self;
    return makeWith(client.postprocess, (request) => Effect.map(client.preprocess(request), f));
});
/** @internal */
export const mapRequestEffect = dual(2, (self, f) => {
    const client = self;
    return makeWith(client.postprocess, (request) => Effect.flatMap(client.preprocess(request), f));
});
/** @internal */
export const mapRequestInput = dual(2, (self, f) => {
    const client = self;
    return makeWith(client.postprocess, (request) => client.preprocess(f(request)));
});
/** @internal */
export const mapRequestInputEffect = dual(2, (self, f) => {
    const client = self;
    return makeWith(client.postprocess, (request) => Effect.flatMap(f(request), client.preprocess));
});
/** @internal */
export const retry = dual(2, (self, policy) => transformResponse(self, Effect.retry(policy)));
/** @internal */
export const retryTransient = dual(2, (self, options) => {
...
const isTransientHttpError = (error) => Error.isHttpClientError(error) &&
    ((error._tag === "RequestError" && error.reason === "Transport") ||
        (error._tag === "ResponseError" && isTransientResponse(error.response)));
const isTransientResponse = (response) => response.status === 408 ||
    response.status === 429 ||
    response.status === 500 ||
    response.status === 502 ||
    response.status === 503 ||
    response.status === 504;
/** @internal */
export const tap = dual(2, (self, f) => transformResponse(self, Effect.tap(f)));
/** @internal */
export const tapError = dual(2, (self, f) => transformResponse(self, Effect.tapError(f)));
/** @internal */
export const tapRequest = dual(2, (self, f) => {
    const client = self;
    return makeWith(client.postprocess, (request) => Effect.tap(client.preprocess(request), f));
});
/** @internal */
export const withCookiesRef = dual(2, (self, ref) => {
    const client = self;
    return makeWith((request) => Effect.tap(client.postprocess(request), (response) => Ref.update(ref, (cookies) => Cookies.merge(cookies, response.cookies))), (request) => Effect.flatMap(client.preprocess(request), (request) => Effect.map(Ref.get(ref), (cookies) => Cookies.isEmpty(cookies)
        ? request
        : internalRequest.setHeader(request, "cookie", Cookies.toCookieHeader(cookies)))));
});
/** @internal */
export const followRedirects = dual((args) => isClient(args[0]), (self, maxRedirects) => {
    const client = self;
    return makeWith((request) => {
        const loop = (request, redirects) => Effect.flatMap(client.postprocess(Effect.succeed(request)), (response) => response.status >= 300 && response.status < 400 && response.headers.location &&
            redirects < (maxRedirects ?? 10)
            ? loop(internalRequest.setUrl(request, new URL(response.headers.location, response.request.url)), redirects + 1)
