    ...
    return WorkerManager.of({
        ...
        spawn({ encode, initialMessage }) {
            return Effect.gen(function* () {
                ...
                }).pipe(Effect.onError((cause) => Effect.forEach(requestMap.values(), (mailbox) => Deferred.DeferredTypeId in mailbox
                    ? Deferred.failCause(mailbox, cause)
                    : mailbox.failCause(cause))), Effect.tapErrorCause(Effect.logWarning), Effect.retry(Schedule.spaced(1000)), Effect.annotateLogs({
                    package: "@effect/platform",
                    module: "Worker"
                }), Effect.interruptible, Effect.forkScoped);
                yield* Effect.addFinalizer(() => Effect.zipRight(Effect.forEach(requestMap.values(), (mailbox) => Deferred.DeferredTypeId in mailbox
                ...
                const executeAcquire = (request, makeMailbox) => Effect.withFiberRuntime((fiber) => {
                    ...
                    return makeMailbox.pipe(Effect.tap((mailbox) => {
                        ...
                        return wrappedEncode(request).pipe(Effect.tap((payload) => backing.send([
                            id,
                            0,
                            payload,
                            span._tag === "Some" ? [span.value.traceId, span.value.spanId, span.value.sampled] : undefined
                        ], collector.unsafeRead())), Effect.catchAllCause((cause) => Mailbox.isMailbox(mailbox)
                            ? mailbox.failCause(cause)
                            : Deferred.failCause(mailbox, cause)));
                ...
                const executeEffect = (request) => Effect.acquireUseRelease(executeAcquire(request, Deferred.make()), ({ mailbox }) => Deferred.await(mailbox), executeRelease);
                yield* Deferred.await(readyLatch);
                if (initialMessage) {
                    yield* Effect.sync(initialMessage).pipe(Effect.flatMap(executeEffect), Effect.mapError((cause) => new WorkerError({ reason: "spawn", cause })));
                }
...
export const makeSerialized = (options) => Effect.gen(function* () {
    ...
    const backing = yield* manager.spawn({
        ...options,
        encode(message) {
            return Effect.mapError(Schema.serialize(message), (cause) => new WorkerError({ reason: "encode", cause }));
        }
    });
