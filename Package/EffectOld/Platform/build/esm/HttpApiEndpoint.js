 ...
 * @category type ids
 */
export const TypeId = Symbol.for("@effect/platform/HttpApiEndpoint");
/**
 * @since 1.0.0
 * @category guards
 */
...
const Proto = {
    ...
    addError(schema, annotations) {
        return makeProto({
            ...this,
            errorSchema: HttpApiSchema.UnionUnify(this.errorSchema, annotations?.status ? schema.annotations(HttpApiSchema.annotations({ status: annotations.status })) : schema)
        });
    },
    setPayload(schema) {
        return makeProto({
