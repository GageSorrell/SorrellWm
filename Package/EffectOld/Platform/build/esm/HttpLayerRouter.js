 ...
 * @category HttpRouter
 */
export const TypeId = Symbol.for("@effect/platform/HttpLayerRouter/HttpRouter");
/**
 * @since 1.0.0
 * @category HttpRouter
 */
export const HttpRouter = Context.GenericTag("@effect/platform/HttpLayerRouter");
/**
 * @since 1.0.0
 * @category HttpRouter
 */
    ...
    return HttpRouter.of({
        ...
        prefixed(prefix) {
            return HttpRouter.of({
                ...
                add: (method, path, handler, options) => addAll([
                    makeRoute({
                        method,
        ...
        asHttpEffect() {
            let handler = Effect.withFiberRuntime((fiber) => {
                ...
                return Effect.locally((route.uninterruptible ?
                    route.handler :
                    Effect.interruptible(route.handler)), FiberRef.currentContext, Context.unsafeMake(contextMap));
            });
 ...
 */
export class RouterConfig extends Context.Reference()("@effect/platform/HttpLayerRouter/RouterConfig", {
 ...
 * @category HttpRouter
 */
export const layer = Layer.effect(HttpRouter, make);
/**
 * @since 1.0.0
 * @category HttpRouter
 */
 ...
 * @category Route
 */
export const RouteTypeId = Symbol.for("@effect/platform/HttpLayerRouter/Route");
 ...
 * @category Route
 */
export const prefixRoute = dual(2, (self, prefix) => makeRoute({
    ...self,
 ...
 * @category Middleware
 */
export const MiddlewareTypeId = Symbol.for("@effect/platform/HttpLayerRouter/Middleware");
/**
...
const makeMiddleware = (middleware, options) => options?.global ?
    ...
    : new MiddlewareImpl(Effect.isEffect(middleware) ?
        Layer.scopedContext(Effect.map(middleware, (fn) => Context.unsafeMake(new Map([[fnContextKey, fn]])))) :
        Layer.succeedContext(Context.unsafeMake(new Map([[fnContextKey, middleware]]))));
let middlewareId = 0;
 ...
 * @category Middleware
 */
export const cors = (options) => middleware(HttpMiddleware.cors(options), { global: true });
/**
 ...
 * @category Middleware
 */
export const disableLogger = middleware(HttpMiddleware.withLoggerDisabled).layer;
/**
 * ```ts
...
const existingRoutesMap = new WeakMap();
/**
...
export const toWebHandler = (appLayer, options) => {
    ...
    const RouterLayer = Layer.provideMerge(appLayer, options?.routerConfig
        ? Layer.provide(layer, Layer.succeed(RouterConfig, options.routerConfig))
        : layer);
