...
import { Array, Brand, Option, String } from "effect";
...
export const UnsafeDefined = <A>(Value: A | undefined): A => Value!;

export const OptionGetUnsafe = <A>(Value: Option.Option<A>): A => Option.getOrUndefined(Value)!;

export const MakeUnsafe = <ArgumentVectorType extends ReadonlyArray<unknown>, A>(
    Getter: (...Arguments: ArgumentVectorType) => Option.Option<A>
): ((...Arguments: ArgumentVectorType) => A) => (...Arguments: ArgumentVectorType) =>
    Option.getOrUndefined(Getter(...Arguments))!;
