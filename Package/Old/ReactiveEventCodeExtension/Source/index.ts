...
import { type CliConfig, DeclareEvents, GetConfig } from "reactive-event-cli";
import {
    type Disposable,
        ...
        async (Document: TextDocument) =>
        {
            ...
            if (UsesEventDecl)
            {
                ...
                try
                {
                    ...
                    // await RunNpmCommand("npm exec reactive-event-cli declare-events");
                    await DeclareEvents(dirname(Document.fileName));
                }
                catch
                {
                    /* eslint-disable-next-line @stylistic/max-len */
                    OutputChannel.appendLine(`Could not find an reactive-event-cli config file corresponding to module ${ Document.fileName }.`);
                }
            }
        }
    );
