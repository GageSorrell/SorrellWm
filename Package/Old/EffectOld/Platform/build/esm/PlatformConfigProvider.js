 ...
 */
export const fromFileTree = (options) => Effect.Do.pipe(Effect.bind("path", () => Path.Path), Effect.bind("fs", () => FileSystem.FileSystem), Effect.map(({ fs, path }) => {
    ...
    const handlePlatformError = (pathSegments) => (error) => error._tag === "SystemError" && error.reason === "NotFound"
