...
<html>
  ...
  <body>
    ...
    ${source._tag === "Cdn"
        ? `<script src="${`https://cdn.jsdelivr.net/npm/@scalar/api-reference@${source.version ?? "latest"}/dist/browser/standalone.min.js`}" crossorigin></script>`
 ...
 * @category layers
 */
export const layerHttpLayerRouter = Effect.fnUntraced(function* (options) {
 ...
 * @category layers
 */
export const layerHttpLayerRouterCdn = Effect.fnUntraced(function* (options) {
    const router = yield* HttpLayerRouter.HttpRouter;
    const handler = makeHandler({
        ...options,
        source: {
