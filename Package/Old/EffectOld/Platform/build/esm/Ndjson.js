 ...
 * @category type ids
 */
export const ErrorTypeId = Symbol.for("@effect/platform/Ndjson/NdjsonError");
...
const filterEmpty = Chunk.filter((line) => line.length > 0);
...
export const unpackString = (options) => {
    const lines = Channel.splitLines().pipe(options?.ignoreEmptyLines === true ?
 ...
 */
export const duplex = dual((args) => Channel.isChannel(args[0]), (self, options) => Channel.pipeTo(Channel.pipeTo(pack(), self), unpack(options)));
/**
 * @since 1.0.0
 * @category combinators
 */
export const duplexString = dual((args) => Channel.isChannel(args[0]), (self, options) => Channel.pipeTo(Channel.pipeTo(packString(), self), unpackString(options)));
/**
 * @since 1.0.0
 * @category combinators
 */
export const duplexSchema = dual(2, (self, options) => ChannelSchema.duplexUnknown(duplex(self, options), options));
/**
 * @since 1.0.0
 * @category combinators
 */
export const duplexSchemaString = dual(2, (self, options) => ChannelSchema.duplexUnknown(duplexString(self, options), options));
