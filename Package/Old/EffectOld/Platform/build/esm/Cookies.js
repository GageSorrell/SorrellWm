 ...
 * @category type ids
 */
export const TypeId = Symbol.for("@effect/platform/Cookies");
/**
 * @since 1.0.0
 ...
 * @category type ids
 */
export const CookieTypeId = Symbol.for("@effect/platform/Cookies/Cookie");
/**
 * @since 1.0.0
 * @category type ids
 */
export const ErrorTypeId = Symbol.for("@effect/platform/Cookies/CookieError");
/**
 * @since 1.0.0
 * @category errors
 */
 ...
 */
export const setCookie = dual(2, (self, cookie) => fromReadonlyRecord(Record.set(self.cookies, cookie.name, cookie)));
/**
 ...
 * @category combinators
 */
export const setAllCookie = dual(2, (self, cookies) => {
 ...
 * @category combinators
 */
export const merge = dual(2, (self, that) => fromReadonlyRecord({
    ...self.cookies,
    ...that.cookies
}));
/**
 ...
 * @category combinators
 */
export const remove = dual(2, (self, name) => fromReadonlyRecord(Record.remove(self.cookies, name)));
/**
 ...
 * @category combinators
 */
export const get = dual((args) => isCookies(args[0]), (self, name) => Record.get(self.cookies, name));
/**
 * Get a cookie from a Cookies object
 *
 * @since 1.0.0
 * @category combinators
 */
export const getValue = dual((args) => isCookies(args[0]), (self, name) => Option.map(Record.get(self.cookies, name), (cookie) => cookie.value));
/**
 * Add a cookie to a Cookies object
 *
 * @since 1.0.0
 * @category combinators
 */
export const set = dual((args) => isCookies(args[0]), (self, name, value, options) => Either.map(makeCookie(name, value, options), (cookie) => fromReadonlyRecord(Record.set(self.cookies, name, cookie))));
/**
 * Add a cookie to a Cookies object
 *
 * @since 1.0.0
 * @category combinators
 */
export const unsafeSet = dual((args) => isCookies(args[0]), (self, name, value, options) => fromReadonlyRecord(Record.set(self.cookies, name, unsafeMakeCookie(name, value, options))));
/**
 * Add multiple cookies to a Cookies object
 *
 * @since 1.0.0
 * @category combinators
 */
export const setAll = dual(2, (self, cookies) => {
    const record = { ...self.cookies };
 ...
 * @category combinators
 */
export const unsafeSetAll = dual(2, (self, cookies) => Either.getOrThrowWith(setAll(self, cookies), identity));
/**
...
export function parseHeader(header) {
    ...
    while (true) {
        ...
        if (terminatorPos === -1)
            terminatorPos = strLen; // This is the last pair
        let eqIdx = header.indexOf("=", pos);
        if (eqIdx === -1)
            break; // No key-value pairs left
