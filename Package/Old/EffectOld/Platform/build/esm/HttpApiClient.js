...
const makeClient = (api, options) => Effect.gen(function* () {
    ...
    const httpClient = options.httpClient.pipe(options?.baseUrl === undefined
        ? identity
...
const Uint8ArrayFromArrayBuffer = Schema.transform(Schema.Unknown, Schema.Uint8ArrayFromSelf, {
    decode(fromA) {
...
const StringFromArrayBuffer = Schema.transform(Schema.Unknown, Schema.String, {
    decode(fromA) {
...
const parseJsonOrVoid = Schema.transformOrFail(Schema.String, Schema.Unknown, {
    ...
    decode: (i, _, ast) => {
        if (i === "")
            return ParseResult.succeed(void 0);
...
const bodyFromPayloadCache = globalValue("@effect/platform/HttpApiClient/bodyFromPayloadCache", () => new WeakMap());
