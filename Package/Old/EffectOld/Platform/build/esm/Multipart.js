 ...
 * @category type ids
 */
export const TypeId = Symbol.for("@effect/platform/Multipart");
/**
 * @since 1.0.0
 * @category Guards
 */
 ...
 * @category Errors
 */
export const ErrorTypeId = Symbol.for("@effect/platform/Multipart/MultipartError");
/**
 * @since 1.0.0
 * @category Errors
 */
 ...
 * @category Schemas
 */
export const FileSchema = Schema.declare(isPersistedFile, {
 ...
 * @category Schemas
 */
export const SingleFileSchema = Schema.transform(FilesSchema.pipe(Schema.itemsCount(1)), FileSchema, {
    strict: true,
    decode: ([file]) => file,
    encode: (file) => [file]
});
 ...
 * @category Parsers
 */
export const makeChannel = (headers, bufferSize = 16) => Channel.acquireUseRelease(Effect.all([
    makeConfig(headers),
    Mailbox.make(bufferSize)
]), ([config, mailbox]) => {
    let partsBuffer = [];
...
class FileImpl extends PartBase {
    ...
    constructor(info, channel) {
        ...
        this.contentEffect = channel.pipe(Channel.pipeTo(collectUint8Array), Channel.run, Effect.mapError((cause) => new MultipartError({ reason: "InternalError", cause })));
    }
    toJSON() {
        return {
            _id: "@effect/platform/Multipart/Part",
...
const defaultWriteFile = (path, file) => Effect.flatMap(FileSystem.FileSystem, (fs) => Effect.mapError(Stream.run(file.content, fs.sink(path)), (cause) => new MultipartError({ reason: "InternalError", cause })));
/**
 * @since 1.0.0
 */
 ...
 * @category fiber refs
 */
export const withLimits = dual(2, (effect, options) => Effect.provide(effect, withLimitsContext(options)));
 ...
 * @category fiber refs
 */
export const withLimitsStream = dual(2, (stream, options) => Stream.provideSomeContext(stream, withLimitsContext(options)));
/**
 * @since 1.0.0
 * @category fiber refs
 */
export class MaxParts extends Context.Reference()("@effect/platform/Multipart/MaxParts", {
 ...
 * @category fiber refs
 */
export const withMaxParts = dual(2, (effect, count) => Effect.provideService(effect, MaxParts, count));
/**
 * @since 1.0.0
 * @category fiber refs
 */
export class MaxFieldSize extends Context.Reference()("@effect/platform/Multipart/MaxFieldSize", {
 ...
 * @category fiber refs
 */
export const withMaxFieldSize = dual(2, (effect, size) => Effect.provideService(effect, MaxFieldSize, FileSystem.Size(size)));
/**
 * @since 1.0.0
 * @category fiber refs
 */
export class MaxFileSize extends Context.Reference()("@effect/platform/Multipart/MaxFileSize", {
    defaultValue: (Option.none)
}) {
}
/**
 ...
 * @category fiber refs
 */
export const withMaxFileSize = dual(2, (effect, size) => Effect.provideService(effect, MaxFileSize, Option.map(size, FileSystem.Size)));
/**
 * @since 1.0.0
 * @category fiber refs
 */
export class FieldMimeTypes extends Context.Reference()("@effect/platform/Multipart/FieldMimeTypes", {
    defaultValue: constant(Chunk.make("application/json"))
}) {
}
/**
 * @since 1.0.0
 * @category fiber refs
 */
export const withFieldMimeTypes = dual(2, (effect, mimeTypes) => Effect.provideService(effect, FieldMimeTypes, Chunk.fromIterable(mimeTypes)));
