...
/** @internal */
export const tag = GenericTag("@effect/platform/FileSystem");
/** @internal */
...
export const make = (impl) => {
    return tag.of({
        ...
        stream: (path, options) => pipe(impl.open(path, { flag: "r" }), options?.offset ?
            Effect.tap((file) => file.seek(options.offset, "start")) :
            identity, Effect.map((file) => stream(file, options)), Stream.unwrapScoped),
        sink: (path, options) => pipe(impl.open(path, { flag: "w", ...options }), Effect.map((file) => Sink.forEach((_) => file.writeAll(_))), Sink.unwrapScoped),
...
/** @internal */
const stream = (file, { bufferSize = 16, bytesToRead: bytesToRead_, chunkSize: chunkSize_ = Size(64 * 1024) } = {}) => {
