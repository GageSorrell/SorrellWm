...
/** @internal */
export const loggerDisabled = globalValue(Symbol.for("@effect/platform/HttpMiddleware/loggerDisabled"), () => FiberRef.unsafeMake(false));
/** @internal */
export const withLoggerDisabled = (self) => Effect.zipRight(FiberRef.set(loggerDisabled, true), self);
/** @internal */
export const currentTracerDisabledWhen = globalValue(Symbol.for("@effect/platform/HttpMiddleware/tracerDisabledWhen"), () => FiberRef.unsafeMake(constFalse));
/** @internal */
export const withTracerDisabledWhen = dual(2, (self, pred) => Layer.locally(self, currentTracerDisabledWhen, pred));
/** @internal */
export const withTracerDisabledWhenEffect = dual(2, (self, pred) => Effect.locally(self, currentTracerDisabledWhen, pred));
/** @internal */
export const withTracerDisabledForUrls = dual(2, (self, urls) => Layer.locally(self, currentTracerDisabledWhen, (req) => urls.includes(req.url)));
/** @internal */
export const SpanNameGenerator = Context.Reference()("@effect/platform/HttpMiddleware/SpanNameGenerator", {
    defaultValue: () => (request) => `http.server ${request.method}`
});
/** @internal */
export const withSpanNameGenerator = dual(2, (self, f) => Layer.provide(self, Layer.succeed(SpanNameGenerator, f)));
/** @internal */
...
/** @internal */
export const tracer = make((httpApp) => Effect.withFiberRuntime((fiber) => {
    const request = Context.unsafeGet(fiber.currentContext, ServerRequest.HttpServerRequest);
...
/** @internal */
export const xForwardedHeaders = make((httpApp) => Effect.updateService(httpApp, ServerRequest.HttpServerRequest, (request) => {
