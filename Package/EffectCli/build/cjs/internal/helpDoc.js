...
const toAnsiDocInternal = self => {
  switch (self._tag) {
    ...
    case "DescriptionList":
      {
        const definitions = self.definitions.map(([span, doc]) => Doc.cats([Doc.annotate(InternalSpan.toAnsiDoc(span), Ansi.bold), Doc.indent(toAnsiDocInternal(doc), 2), Doc.hardLine]));
    ...
    case "Sequence":
      {
        return Doc.cat(toAnsiDocInternal(self.left), toAnsiDocInternal(self.right));
      }
  }
};
