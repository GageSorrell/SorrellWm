...
type NpmPublishResult =
    {
        MustAuthenticate: boolean;
        OnExit: Promise<PublishResult>;
    };

function BeginNpmPublish(): Promise<NpmPublishResult>
{
    return new Promise<NpmPublishResult>((Resolve: ((Value: NpmPublishResult) => void)): void =>
    {
        ...
        const OnExit: Promise<PublishResult> = new Promise<PublishResult>((
            ResolveOnExit: (Value: PublishResult) => void
        ): void =>
        {
            TerminalProcess.onExit(({ exitCode: ExitCode }: OnExitParameter): void =>
            {
                ResolveOnExit(ExitCode === 0 ? "Success" : Result);
            });
        });
    });
}

// async function RunNpmPublishPublic(): Promise<PublishResult>
// {
//     return new Promise<PublishResult>((Resolve: ((Value: PublishResult) => void)): void =>
//     {
//         const TerminalProcess: IPty = PtySpawn(
//             GetNpmExecutableName(),
//             [ "publish", "--access", "public" ],
...
//                 !EnterSent
//                 && (
//                     Buffer.includes("Press ENTER to open in the browser")
//                     || Buffer.includes("https://www.npmjs.com/")
//                 )
//             )
//             {
//                 EnterSent = true;
//                 TerminalProcess.write("\r");
//                 Result = "MustAuthenticate";
//             }
//         });
...
//         TerminalProcess.onExit(({ exitCode: ExitCode }: OnExitParameter): void =>
//         {
//             Resolve(Result === "Success" && ExitCode === 0 ? "Success" : "UnknownError");
//         });
//     });
// }

export function PublishWithPatchOnVersionConflict(Silent: boolean): (() => Promise<void>)
{
        ...
        await new Listr<Context>([
            ...
            },
            {
                task: async (Context: Context, Task: TaskWrapper): Promise<void | Listr> =>
                {
                    ...
                    // const Result: PublishResult = await RunNpmPublishPublic();
                    const { MustAuthenticate: MustLogIn, OnExit }: NpmPublishResult = await BeginNpmPublish();
                    // @TODO Handle PermissionsError case when `npm login` must be called.
                    // Also handle case in which authentication needs to happen.
                    if (MustLogIn)
                    {
