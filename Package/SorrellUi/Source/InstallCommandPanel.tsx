 ...
 * @since 1.0.0
 */
export const InstallCommandPanel = ({ className: ClassName, defaultPackageManager: DefaultPackageManager = "npm", icons: Icons = {}, packageName: PackageName }: InstallCommandPanelProps): React.JSX.Element =>
{
    ...
    const OnCopy = async (): Promise<void> =>
    {
        ...
        const NextState = Clipboard === undefined ? "error" : await Clipboard.writeText(InstallCommand(ActivePackageManager, PackageName)).then(() => "copied" as const).catch(() => "error" as const);
    ...
    return (
        <div className={ Cn("relative block rounded-md bg-[#101012] p-1 ring-1 ring-inset ring-zinc-700", ClassName) }>
            <div aria-label="Copy install command"
                className="flex min-h-11 w-full cursor-pointer items-center gap-3 px-4 py-1 text-left font-mono text-sm text-zinc-300 transition-colors hover:bg-[#17171A]"
                onClick={ () => void OnCopy() }
                ...
                <div className="relative shrink-0"
                    ...
                    <button aria-expanded={ IsDropdownOpen }
                        ...
                        className="flex cursor-pointer items-center gap-1.5 rounded-sm border-r border-zinc-800 py-0.5 pr-3 pl-1 text-xs text-zinc-300 transition-colors hover:text-white"
                        onClick={ (Event) => { Event.stopPropagation(); SetIsDropdownOpen((Open) => !Open); } }
                        type="button">
                        <span aria-hidden="true">{ Icons[ActivePackageManager] }</span>
                        <span>{ ActivePackageManager }</span>
                        <ChevronDown aria-hidden="true"
                            className={ Cn("size-4 text-zinc-500 transition-transform", IsDropdownOpen ? "rotate-180" : undefined) } />
                    </button>

                    {
                        IsDropdownOpen ?
                            <ul className="absolute top-full left-0 z-20 mt-2 w-36 overflow-hidden rounded-md border border-zinc-800 bg-zinc-900 shadow-xl"
                                role="listbox">
                                {
                                        ...
                                        return (
                                            <li key={ PackageManagerOption }>
                                                <button aria-selected={ IsActive }
                                                    className={ Cn("flex w-full cursor-pointer items-center gap-2.5 px-3 py-2 text-left text-sm transition-colors hover:bg-zinc-800", IsActive ? "text-white" : "text-zinc-400") }
                                                    onClick={ (Event) => { Event.stopPropagation(); SetActivePackageManager(PackageManagerOption); SetIsDropdownOpen(false); } }
                                                    role="option"
                                                    type="button">
                ...
                <span className="flex-1 truncate">{ InstallCommand(ActivePackageManager, PackageName) }</span>
