[electron-reactive-event](../index.md) / Internal

# Internal

The namespaces and types that are used internally, but are not exported.

## Namespaces

| Namespace                                        | Description |
| ------------------------------------------------ | ----------- |
| [Callback](namespaces/Callback/index.md)         | -           |
| [Event](namespaces/Event/index.md)               | -           |
| [Factory](namespaces/Factory/index.md)           | -           |
| [Registrar](namespaces/Registrar/index.md)       | -           |
| [Serializable](namespaces/Serializable/index.md) | -           |
| [Utility](namespaces/Utility/index.md)           | -           |
| [Validation](namespaces/Validation/index.md)     | -           |
