...
import type {
    ...
    LoggerFieldOptions,
    LoggerFormat,
    LoggerFormatOptions } from "./logger.interface.js";
import { ProcessOutput, splat } from "@utils/index.js";
import { EOL } from "os";
...
export class ListrLogger<Levels extends string = string>
{
    ...
    {
        const output: string = this.format(level, message, options);
    ...
    public suffix(message: string, ...suffixes: Array<LoggerField>): string
    {
        suffixes.filter(Boolean).forEach((suffix: LoggerField): void =>
        {
    ...
    public prefix(message: string, ...prefixes: Array<LoggerField>): string
    {
        prefixes.filter(Boolean).forEach((prefix: LoggerField) =>
        {
    ...
    public icon(level: Levels, icon?: string | false): string
    {
        ...
        if (!icon)
        {
            type Eye = Partial<Record<string | Levels, string | (() => string)>>[Levels];
            const i: Eye = this.options.icon?.[level];
    ...
    protected format(level: Levels, message: string | Array<string>, options?: LoggerFieldOptions): string
    {
        ...
        message = this.splat((message as Array<string>).shift(), ...message)
            .toString()
            .split(EOL)
    ...
    protected style(level: Levels, message: string): string
    {
        ...
        const icon: string = this.icon(level, !this.options.useIcons && this.wrap(level));
