import type { ListrOptions } from "@root/index.js"
import { ListrErrorTypes, Listr } from "@root/index.js"
...
describe('error collection', () => {
  ...
  it('should save the context on error', async() => {
    ...
    expect(crash).toBeTruthy()
    expect(task.errors.length).toBe(1)
    expect(task.Context).toStrictEqual(Context)
    expect(task.errors[0]).toMatchObject({ message, Context })
  })
  ...
  it('should save the context on error while having circular dependencies', async() => {
    ...
    expect(task.errors[0].Context).toMatchObject({ test: true, myself: { test: true } })
  })
  ...
  it.each<ListrOptions['collectErrors']>(['minimal', 'full'])('should properly record the context in the given error with collect errors: %s', async(collectErrors) => {
    ...
    if (collectErrors === 'full') {
      expect(task.errors[0].Context).toMatchObject({ test1: true })
      expect(task.errors[1].Context).toMatchObject({ test1: true, test2: true })
      expect(task.errors[2].Context).toMatchObject({
        test1: true,
        test2: true,
        test3: true
      })
    } else {
      expect(task.errors[0].Context).toBe(undefined)
      expect(task.errors[1].Context).toBe(undefined)
      expect(task.errors[2].Context).toBe(undefined)
    }
  })
