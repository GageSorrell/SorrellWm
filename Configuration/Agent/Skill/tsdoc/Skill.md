---
name: tsdoc
description: Write, insert, or update TSDoc comments such that the comments satisfy the writing style herein, and the monorepo-wide ESLint rules.
---
...
When updating TSDoc comments,

1. Inspect the declaration, implementation, nearby tests, and nearby TSDoc before editing.
...
Use a normal multiline TSDoc comment in TypeScript source,
...
- Write all public TSDoc prose in English.
...
- Do not use ad hoc bold headings such as `# Notes`; only the standard TSDoc tags are allowed.
...
- Avoid overlinking in prose. Use `{@link Symbol}` only when navigation to
  ...
  `Context`.
- Do not document module-level comments; module TSDoc is ignored by this rule.
- `@internal` means the item is ignored; do not rewrite it as public docs.
- Default exports are ignored by this rule and do not need TSDoc.
